...
number is published it is stored forever and never disappears — solving the
"I didn't look today, tomorrow it's gone" problem. Revisions update in place.

Forecast (consensus) is the one field FRED doesn't provide; it's left null unless
you later wire a calendar source. Everything else is solid and persistent.
...
def fred_series(series_id, n=25):
    ...
    return [{"date": o["date"], "value": float(o["value"])} for o in obs]  # newest first
...
def build_events():
    ...
    for ind in c["indicators"]:
        try:
            ...
            if len(obs) < 2:
                continue
            for i in range(min(12, len(obs) - 1)):  # last ~12 releases
...
def merge_persist(new_events):
    ...
    if os.path.exists(OUT):
        for e in json.load(open(OUT)).get("events", []):
            old[e["id"]] = e
    for e in new_events:
        if e["id"] in old:
            # keep existing but refresh actual/forecast/previous if newly present
            for k in ("actual", "forecast", "previous"):
                if e.get(k) is not None:
                    old[e["id"]][k] = e[k]
        else:
            old[e["id"]] = e
    events = sorted(old.values(), key=lambda x: x["date"])
...
def main():
    if not FRED_KEY:
        print("No FRED_KEY set — skipping FRED pull, keeping existing archive + FOMC dates.")
        new = [e for e in build_events() if e["code"] == "FOMC"]
    else:
        new = build_events()
    n = merge_persist(new)
    print(f"Econ done. {n} events archived.")
